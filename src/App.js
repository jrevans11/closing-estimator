    ...
    >
      ...
      {file ? (
        <>
          ...
          <span style={{ fontSize: 13, color: "var(--color-text-success)", fontWeight: 500 }}>{file.name}</span>
        ...
        <>
          ...
          <span style={{ fontSize: 14, color: "var(--color-text-primary)", fontWeight: 500 }}>{label}</span>
          <span style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Click or drag and drop PDF</span>
        </>
      )}
    </div>
  );
...
export default function App() {
  ...
  const generate = async () => {
    ...
    try {
      setLoadMsg("Extracting text from inspection report...");
      const inspText = await extractTextFromPDF(inspFile);
      setLoadMsg("Extracting text from repair addendum...");
      const addText = await extractTextFromPDF(addFile);
      setLoadMsg("Analyzing documents and building estimate...");
      ...
      const userText =
        "Below is the text extracted from the home inspection report and repair addendum. Generate a closing repairs estimate for Owners Choice Construction.\n\n" +
        "Only include items within a general contractor scope. Cross-reference the addendum items with the inspection report details to write accurate scope descriptions.\n\n" +
        (clientName ? "Client name: " + clientName + "\n" : "Extract client name from the documents.\n") +
        (clientPhone ? "Client phone: " + clientPhone + "\n" : "") +
        (clientEmail ? "Client email: " + clientEmail + "\n" : "") +
        extra +
        "\n\n=== HOME INSPECTION REPORT ===\n" +
        inspText.slice(0, 60000) +
        "\n\n=== REPAIR ADDENDUM ===\n" +
        addText.slice(0, 10000) +
      ...
      {step === "upload" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Home Inspection Report</div>
          ...
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-lg)", padding: "14px 16px", marginBottom: 12, border: "0.5px solid var(--color-border-tertiary)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              {[
                ...
                <div key={label}>
                  ...
                  <input type="text" value={val} onChange={(e) => setter(e.target.value)} placeholder={label} style={{ width: "100%", fontSize: 13, boxSizing: "border-box" }} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <button onClick={() => setShowWufoo(!showWufoo)} style={{ fontSize: 13, padding: "6px 12px" }}>
              <i className="ti ti-mail" style={{ marginRight: 6 }} />
              {showWufoo ? "Hide" : "Paste Wufoo email"} (optional)
              <i className={"ti ti-chevron-" + (showWufoo ? "up" : "down")} style={{ marginLeft: 6 }} />
            </button>
            {showWufoo && (
              <div style={{ marginTop: 10 }}>
                <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 6 }}>
                  Paste the Wufoo inquiry email body here. Client info and property address will be extracted automatically.
                </label>
                <textarea
                  value={wufoo}
                  ...
                  rows={6}
                  style={{ width: "100%", fontSize: 13, fontFamily: "var(--font-sans)", boxSizing: "border-box", resize: "vertical", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-secondary)", padding: "8px 10px", background: "var(--color-background-secondary)", color: "var(--color-text-primary)" }}
                />
              </div>
            )}
          </div>
          ...
          {error && (
            <div style={{ padding: "10px 14px", background: "var(--color-background-danger)", borderRadius: "var(--border-radius-md)", fontSize: 13, color: "var(--color-text-danger)", marginBottom: 12 }}>
          ...
          <button
            ...
            style={{ width: "100%", padding: "11px 0", fontSize: 14, fontWeight: 500, cursor: inspFile && addFile && !loading ? "pointer" : "not-allowed", opacity: inspFile && addFile && !loading ? 1 : 0.5 }}
          >
            {loading ? (
              <span>
                <i className="ti ti-loader-2" style={{ marginRight: 6, display: "inline-block", animation: "spin 1s linear infinite" }} />
                {loadMsg}
              </span>
            ) : (
              <span><i className="ti ti-sparkles" style={{ marginRight: 6 }} />Generate Estimate</span>
            )}
          </button>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <button onClick={reset} style={{ fontSize: 13, padding: "7px 14px" }}>
              <i className="ti ti-arrow-left" style={{ marginRight: 4 }} />New Estimate
            </button>
            <button onClick={printEstimate} style={{ fontSize: 13, padding: "7px 18px", fontWeight: 500 }}>
              <i className="ti ti-printer" style={{ marginRight: 6 }} />Print / Save as PDF
            </button>
          </div>

          <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "20px 24px", fontSize: 13 }}>
            <div style={{ display: "flex", justifyContent: "space-between", paddingBottom: 14, borderBottom: "0.5px solid var(--color-border-tertiary)", marginBottom: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, paddingBottom: 14, borderBottom: "0.5px solid var(--color-border-tertiary)", marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 11, color: "var(--color-text-secondary)", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 3 }}>Prepared By</div>
                <div style={{ fontWeight: 500 }}>Jason Evans</div>
                <div>Owners Choice Construction</div>
                <div>(864) 252-4999</div>
                <div style={{ color: "var(--color-text-info)" }}>jason@ownerschoiceconstruction.com</div>
                <div>3122 Wade Hampton Blvd, Taylors, SC 29687</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: "var(--color-text-secondary)", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 3 }}>Prepared For</div>
                {resolved.name && <div style={{ fontWeight: 500 }}>{resolved.name}</div>}
                <div>{estimate.property_address}</div>
                {resolved.phone && <div>{resolved.phone}</div>}
                {resolved.email && <div style={{ color: "var(--color-text-info)" }}>{resolved.email}</div>}
              </div>
            </div>

            <div style={{ fontSize: 11, color: "var(--color-text-secondary)", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 2 }}>Closing Repairs Estimate Details</div>
            <div style={{ fontWeight: 500, fontSize: 14, marginBottom: 14 }}>{estimate.property_address}</div>

            <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid var(--color-border-primary)", marginBottom: 2 }}>
              <span style={{ fontSize: 11, color: "var(--color-text-secondary)", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Description</span>
              <span style={{ fontSize: 11, color: "var(--color-text-secondary)", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Total</span>
            </div>
            ...
            {estimate.line_items.map((item, i) => (
              <div key={i} style={{ padding: "10px 0", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                ...
                {item.notes && <div style={{ marginTop: 5, fontSize: 12, color: "var(--color-text-secondary)", fontStyle: "italic" }}>{item.notes}</div>}
              </div>
            ))}

            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", borderTop: "1px solid var(--color-border-primary)", fontWeight: 500, fontSize: 14, marginTop: 2 }}>
              <span>TOTAL</span>
          ...
          {estimate.skipped_items && estimate.skipped_items.length > 0 && (
            <div style={{ marginTop: 12, padding: "12px 14px", background: "var(--color-background-warning)", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-warning)" }}>
              <div style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-warning)", marginBottom: 6 }}>
                <i className="ti ti-info-circle" style={{ marginRight: 4 }} />
                Items excluded from estimate (outside scope or not applicable):
              </div>
              {estimate.skipped_items.map((s, i) => (
                <div key={i} style={{ fontSize: 12, color: "var(--color-text-warning)", marginTop: 2 }}>• {s}</div>
              ))}
            </div>
          )}
        </div>
